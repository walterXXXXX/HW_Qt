...
void MainWindow::on_pb_start_clicked()
{
    ...
    connect(this, &MainWindow::sig_ShowGrapchic, this, &MainWindow::showGraphic);
    ...
    auto findMax = [&](QVector<double> res){
                                                ...
                                                for (int i = 0; i < x.size(); i++) {
                                                    x[i] = static_cast<double>(i) / 1000;
                                                    y[i] = res[i];
                                                }

                                                emit sig_ShowGrapchic(x, y);
...
void MainWindow::showGraphic(QVector<double> x, QVector<double> y)
{
    gf->setWindowTitle("Канал " + ui->cmB_numCh->currentText());
    gf->clearGraphic();
    gf->addGraphic(x, y);
    gf->show();
}
